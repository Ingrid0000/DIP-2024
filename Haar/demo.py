...
def main():
    ...
    ref = Image.open("/home/vboxuser/DIP/B&W_06.png")
    ...
    Image.fromarray(hwt.clip(0,255).astype(np.uint8)).save("06_wavelets_for.png") # save visualization
    cv2.imwrite('06_wavelets_for.jp2', hwt.clip(0,255).astype(np.uint8))
    Image.fromarray(inv.clip(0,255).astype(np.uint8)).save("06_wavelets_inv.png") # save reconstructed image
    cv2.imwrite('06_wavelets_inv.jp2', inv.clip(0,255).astype(np.uint8))
